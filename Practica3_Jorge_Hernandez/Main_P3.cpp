...
int main() {
 ...
 //     
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
		view = glm::translate(view, glm::vec3(0.0f, 0.0f, -15.0f)); //Perspectiva
		...
		model = glm::rotate(model, 0.5f, glm::vec3(3.0f, 1.0f, 2.0f)); // use to compare orthographic and perspective projection
